...
# ═══════════════════════════════════════════════════════════════════════════════
# CLASS WRAPPER (for object-oriented interface)
# ═══════════════════════════════════════════════════════════════════════════════

class TriadHysteresisController:
    """
    Object-oriented wrapper for the TRIAD hysteresis system.
    Provides class-based interface while using module-level state.
    """
    
    def __init__(self, initial_z: float = 0.80):
        """Initialize controller with starting z-coordinate."""
        reset_triad_state(initial_z)
    
    def reset(self, z: float = 0.80) -> 'TriadHysteresisController':
        """Reset the TRIAD state."""
        reset_triad_state(z)
        return self
    
    def update(self, z: float) -> Dict:
        """Update with new z-coordinate (alias for step)."""
        return step(z)
    
    def step(self, z: float) -> Dict:
        """Execute one step of the hysteresis state machine."""
        return step(z)
    
    @property
    def state(self) -> TriadState:
        """Get current TRIAD state."""
        return get_triad_state()
    ...
    def completions(self) -> int:
        """Alias for crossings (for compatibility)."""
        return get_crossings()
    ...
    def drive_to_unlock(self, max_steps: int = 500) -> Dict:
        """Drive the system to TRIAD unlock."""
        return drive_to_unlock(max_steps)
    
    def __repr__(self) -> str:
        state = self.state
        return f"TriadHysteresisController(z={state.z:.4f}, crossings={state.crossings}, unlocked={state.unlocked})"
